...
completed runs at seed \num{42}. The \num{7} detectors that could not be
reproduced faithfully under the shared protocol are excluded from the headline
ranking and reported separately in \cref{sec:results-classd}. The section is
organised as an overall leaderboard, a per-family (U versus M) breakdown, a
per-category analysis, the global-versus-per-series normalisation dichotomy, an
efficiency analysis, a statistical-significance test, and the class-D appendix
...
We state one caveat honestly. The test uses complete-case pruning: only datasets
on which \emph{all} \num{17} detectors produced a score enter the test. Because SRS
is univariate-seasonal and its multivariate run is still pending, the complete-case
pool reduces to $N = \num{250}$ datasets, all univariate; the significance result
is therefore effectively univariate-complete. Given the identical rank ordering
across both families (\cref{tab:tsb_split}) we do not expect the multivariate data
to alter the conclusion, but the definitive combined-family Friedman test awaits
the SRS multivariate run.

\subsection{Class-D appendix results}\label{sec:results-classd}
Seven detectors could not be reproduced faithfully under the shared streaming
protocol --- per-window shuffled evaluation, a frozen ResNet backbone, and no
auxiliary outlier corpus --- and are excluded from the headline ranking rather
than reported under an implementation that does not match the source paper
(\cref{tab:classd}). Three augmented-training methods (Outlier Exposure, DivOE,
DiverseMix) require training against an auxiliary outlier set that the protocol
does not provide; two temporal/drift methods (AE-ADWIN-LSTM, DIVERSIFY) require an
ordered stream or retrain the feature extractor, both incompatible with a frozen
backbone on shuffled windows; and two (DriftLens, TD-IVDM) collapse under their
per-sample implementations to detectors already present in the benchmark. The
reason and disposition for each is recorded in \cref{tab:classd}; the audit
narrative behind these verdicts is developed in \cref{sec:discussion} and
\cref{app:fidelity}.

For completeness we ran these seven on TSB-StreamingAD-U under the evaluation arm
most appropriate to each (the multivariate arms are pending) and report the
outcome in \cref{tab:class_d_appendix}. The result reinforces rather than
complicates the headline: no detector in this group rises clearly above chance and
none is a winner. Their mean AUROC clusters between \num{0.398} and \num{0.662},
with the strongest arm (TD-IVDM as generic KDE density, \num{0.662}) still well
below the feature-manifold leaders and the weakest (DIVERSIFY with an energy score,
\num{0.398}) inverted; the augmented-training methods hover near \num{0.44}--\num{0.62}
depending on whether the head only or the full network is fine-tuned. Evaluated
even under their own favourable protocols, these methods do not challenge the
feature-manifold family, and the two that reduce to feature-space distance/density
(DriftLens, TD-IVDM) merely re-confirm the winning principle under other names.

\input{tables/class_d_appendix.tex}

\begin{table}[t]
\centering
\caption{The seven detectors excluded from the headline ranking as not faithfully
reproducible under the streaming protocol. Each is audited per-method
(\cref{app:fidelity}); the disposition column states how the benchmark accounts for
it.}
\label{tab:classd}
\small
\begin{tabular}{lll}
\toprule
Detector & Reason it is protocol-incompatible & Disposition \\
\midrule
Outlier Exposure & OE trains on an auxiliary outlier set; none available & Folded into Energy (EBO) \\
DivOE & Needs auxiliary outliers + PGD synthesis + training & Excluded (redundant with Energy) \\
DiverseMix & Needs auxiliary outliers + end-to-end training; at chance either orientation & Excluded; negative result \\
DriftLens & Official score is batch/window-level Fr\'echet drift; per-sample $=$ PCA-Mahalanobis & Excluded (duplicate of Mahalanobis) \\
TD-IVDM & Multi-scale drift pillars need ordered windows; per-sample $=$ generic KDE density & Excluded (would be KDE-density) \\
AE-ADWIN-LSTM & LSTM + ADWIN need an ordered stream; eval shuffles windows & Excluded \\
DIVERSIFY & Adversarial representation learning retrains the extractor; defines no OOD score & Excluded \\
\bottomrule
\end{tabular}
\end{table}
