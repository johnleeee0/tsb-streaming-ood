...
\begin{enumerate}
  ...
  \item \textbf{Friedman complete-case caveat.} Because SRS is univariate-seasonal
  and its multivariate run is pending, the complete-case Friedman pool is $N =
  \num{250}$ univariate datasets; the significance result is effectively
  univariate-complete. The identical rank ordering across families
  (\cref{tab:tsb_split}) makes a reversal on the combined pool very unlikely, but the
  definitive combined-family test awaits the SRS multivariate run.
  ...
  their own protocol-appropriate arms on TSB-U only; their multivariate arms are
  pending, and their behaviour under a protocol they \emph{are} designed for
  (ordered streams, auxiliary outlier corpora, batch-level drift) remains untested
  here. Reporting them separately is the honest choice, but it means the comparison
  to the headline set is indicative, not like-for-like.
